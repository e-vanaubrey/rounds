...
import usePasscodeAuth, {
  getPasscode,
  verifyPasscode
} from "./usePasscodeAuth";
  ...
  describe("on first render", () => {
    ...
    it("should not return a user when the app code is invalid", async () => {
      ...
      window.fetch = jest.fn(() =>
        Promise.resolve({
          status: 401,
          json: () => Promise.resolve({ type: "errorMessage" })
        })
      );
    ...
    it("should return an error when an invalid passcode is submitted", async () => {
      ...
      window.fetch = jest.fn(() =>
        Promise.resolve({
          status: 401,
          json: () =>
            Promise.resolve({ error: { message: "passcode incorrect" } })
        })
      );
      const { result, waitForNextUpdate } = renderHook(usePasscodeAuth, {
        wrapper
    ...
    it("should return an error when an expired passcode is submitted", async () => {
      ...
      window.fetch = jest.fn(() =>
        Promise.resolve({
          status: 401,
          json: () =>
            Promise.resolve({ error: { message: "passcode expired" } })
        })
      );
      const { result, waitForNextUpdate } = renderHook(usePasscodeAuth, {
        wrapper
...
describe("the verifyPasscode function", () => {
  ...
  it("should call the API with the correct parameters", async () => {
    ...
    expect(window.fetch).toHaveBeenLastCalledWith("/token", {
      body:
        '{"user_identity":"temp-name","room_name":"temp-room","passcode":"123456"}',
